...
(async () => {
    ...
    for (let i = 0; i < needPhone.length; i += chunkSize) {
        ...
        const { data: emails } = await s.from('email_messages').select('contact_id, body, from_email').in('contact_id', ids).eq('direction', 'RECEIVED').not('body', 'is', null).order('sent_at', { ascending: false }).limit(chunkSize * 3);
        if (!emails) continue;
        const byContact = {};
        emails.forEach(e => { if (!byContact[e.contact_id]) byContact[e.contact_id] = []; if (byContact[e.contact_id].length < 3) byContact[e.contact_id].push(e); });
        for (const [cid, ces] of Object.entries(byContact)) {
            ...
            for (const e of ces) {
                ...
                if (phone) {
                    ...
                    results[cid] = { phone, name: c?.name, email: c?.email };
        ...
        if ((i + chunkSize) % 2000 === 0) process.stderr.write('Scanned ' + (i + chunkSize) + '...\n');
    }
    ...
    for (const [id, d] of Object.entries(clean)) {
        const { error } = await s.from('contacts').update({ phone: d.phone }).eq('id', id);
        if (!error) applied++;
    }

    console.log('Applied:', applied, 'phone numbers');
    const { count: withPhone } = await s.from('contacts').select('id', { count: 'exact' }).not('phone', 'is', null);
    console.log('Contacts with phone now:', withPhone, '/ 12597');
})();
