...
export function cleanPreview(snippet: string): string {
    if (!snippet) return 'No preview';
    
    // Safety: Truncate input to avoid catastrophic backtracking or slow regex on massive bodies
    const input = snippet.length > 2000 ? snippet.substring(0, 2000) : snippet;
