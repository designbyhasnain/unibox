...
interface AnalyticsClientProps {
    initialData: AnalyticsSuccessResult | null;
    initialDeviceData: DeviceAnalyticsResult | null;
}

/* ── Main client component ────────────────────────────────────────── */
export function AnalyticsClient({ initialData, initialDeviceData }: AnalyticsClientProps) {
    ...
    // Start with loading=false when we have server-side initial data
    ...
    // Track whether this is the first render after hydration with default filters.
    // If so and we already have initialData for default params, skip the refetch.
    const isFirstHydratedFetchRef = useRef(true);

    useEffect(() => {
        if (!isHydrated) return;

        // On first hydration, if the filters still match the server-rendered defaults
        // (ALL accounts, ALL managers, and the default date range), skip refetching
        // since the server already provided fresh data.
        if (isFirstHydratedFetchRef.current && initialData) {
            isFirstHydratedFetchRef.current = false;
            // Mark current data as fresh
            const now = Date.now();
            cacheTimestampRef.current = now;
            saveToLocalCache('analytics_data', initialData);
        ...
        fetch();
    }, [startDate, endDate, selectedManager, selectedAccountId, isHydrated]); // eslint-disable-line react-hooks/exhaustive-deps
