    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            try {
                ...
                const [clientsResult, managers, projectsResult] = await Promise.all([
                    getClientsAction(selectedAccountId, 1, 50).catch(() => null),
                    getManagersAction().catch(() => null),
                    getAllProjectsAction(selectedAccountId).catch(() => null),
                ]);

                // Warm up caches
                const clients = clientsResult && 'clients' in clientsResult ? clientsResult.clients : clientsResult;
                const projects = projectsResult && 'projects' in projectsResult ? (projectsResult as any).projects : projectsResult;
                if (clients && managers) {
