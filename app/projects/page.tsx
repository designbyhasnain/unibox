...
export default function ProjectsPage() {
    ...
    const loadData = async () => {
        ...
        try {
            const [pResult, mData, accResult] = await Promise.all([
                getAllProjectsAction(selectedAccountId, currentPage, 100),
                getManagersAction(),
                getAccountsAction()
            ]);

            const pData = Array.isArray(pResult) ? pResult : (pResult as any).projects || [];
            if (!Array.isArray(pResult) && (pResult as any).totalCount !== undefined) {
                setTotalCount((pResult as any).totalCount);
                setTotalPages((pResult as any).totalPages);
                setCurrentPage((pResult as any).page);
            }

            globalProjectsCache = { projects: pData as Project[], managers: mData, clients: [] };
    ...
    // Load clients lazily only when the create modal opens
    const loadClientsForModal = async () => {
        if (clients.length > 0) return;
        try {
            const cResult = await getClientsAction(selectedAccountId, 1, 100);
            const cData = Array.isArray(cResult) ? cResult : (cResult as any).clients || [];
                ...
                <Topbar
                    ...
                    rightContent={
                        <div className="topbar-right-wrapper">
                            <button className="btn btn-primary sm" onClick={() => { loadClientsForModal(); setIsCreateModalOpen(true); }} aria-label="Create new project">
                            ...
                            <PageLoader isLoading={!isHydrated || isLoading} type="list" count={12}>
                                {filteredProjects.length === 0 ? (
                                    ...
                                    <div className="list-area list-area-flex">
                                        <div className="notion-table-wrapper" style={{ overflowX: 'auto' }}>
                                            <div className="notion-table" style={{ minWidth: 1800 }}>
                                                <div className="notion-header">
                                                    <div className="notion-cell" style={{ minWidth: 180 }}>Project</div>
                                                    <div className="notion-cell" style={{ minWidth: 120 }}>Client</div>
                                                    <div className="notion-cell" style={{ minWidth: 90 }}>Status</div>
                                                    <div className="notion-cell" style={{ minWidth: 80 }}>Paid</div>
                                                    <div className="notion-cell" style={{ minWidth: 80 }}>Amount</div>
                                                    <div className="notion-cell" style={{ minWidth: 80 }}>Received</div>
                                                    <div className="notion-cell" style={{ minWidth: 100 }}>Editor</div>
                                                    <div className="notion-cell" style={{ minWidth: 100 }}>Manager</div>
                                                    <div className="notion-cell" style={{ minWidth: 70 }}>Team</div>
                                                    <div className="notion-cell" style={{ minWidth: 80 }}>Priority</div>
                                                    <div className="notion-cell" style={{ minWidth: 85 }}>Date</div>
                                                    <div className="notion-cell" style={{ minWidth: 85 }}>Due</div>
                                                    <div className="notion-cell" style={{ minWidth: 80 }}>Invoice</div>
                                                    <div className="notion-cell" style={{ minWidth: 70 }}>Drive</div>
                                                    <div className="notion-cell" style={{ minWidth: 80 }}>Size</div>
                                                    <div className="notion-cell" style={{ minWidth: 80 }}>Review</div>
                                                    <div className="notion-cell" style={{ minWidth: 60 }}>Code</div>
                                                </div>
                                                <div className="notion-body">
                                                    {filteredProjects.map((p: any) => (
                                                        <div key={p.id} className="notion-row" onClick={() => handleSelectProject(p)} style={{ cursor: 'pointer' }}>
                                                            <div className="notion-cell" style={{ minWidth: 180, display: 'flex', alignItems: 'center', gap: 8 }}>
                                                                <div className="avatar avatar-sm" style={{ background: avatarColor(p.project_name || 'x'), flexShrink: 0 }}>
                                                                    {initials(p.project_name || '?')}
                                                                </div>
                                                                <span style={{ fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.project_name}</span>
                                                            </div>
                                                            <div className="notion-cell" style={{ minWidth: 120 }}>{p.client?.name || (p as any).client_name || (p as any).person || '\u2014'}</div>
                                                            <div className="notion-cell" style={{ minWidth: 90 }}>
                                                                <span className={`notion-badge ${(p as any).status === 'Done' || (p as any).status === 'Delivered' ? 'badge-green' : (p as any).status === 'In Progress' || (p as any).status === 'Downloaded' ? 'badge-blue' : (p as any).status === 'In Revision' ? 'badge-yellow' : (p as any).status === 'on Hold' ? 'badge-red' : 'badge-gray'}`}>
                                                                    {(p as any).status || 'Not Started'}
                                                                </span>
                                                            </div>
                                                            <div className="notion-cell" style={{ minWidth: 80 }}>
                                                                <span className={`notion-badge ${p.paid_status === 'PAID' ? 'badge-green' : p.paid_status === 'PARTIALLY_PAID' ? 'badge-yellow' : 'badge-red'}`}>
                                                                    {p.paid_status === 'PAID' ? 'Paid' : p.paid_status === 'PARTIALLY_PAID' ? 'Partial' : 'Unpaid'}
                                                                </span>
                                                            </div>
                                                            <div className="notion-cell" style={{ minWidth: 80, fontWeight: 600, color: '#10B981' }}>${(p.project_value || 0).toLocaleString()}</div>
                                                            <div className="notion-cell" style={{ minWidth: 80 }}>${((p as any).total_received || 0).toLocaleString()}</div>
                                                            <div className="notion-cell" style={{ minWidth: 100 }}>{(p as any).editor_name || '\u2014'}</div>
                                                            <div className="notion-cell" style={{ minWidth: 100 }}>{(p as any).am_name || p.manager?.name || '\u2014'}</div>
                                                            <div className="notion-cell" style={{ minWidth: 70 }}>{(p as any).team || '\u2014'}</div>
                                                            <div className="notion-cell" style={{ minWidth: 80 }}>
                                                                {p.priority ? <span className={`notion-badge ${p.priority === 'URGENT' ? 'badge-red' : p.priority === 'HIGH' ? 'badge-yellow' : p.priority === 'MEDIUM' ? 'badge-blue' : 'badge-green'}`}>{p.priority}</span> : '\u2014'}
                                                            </div>
                                                            <div className="notion-cell" style={{ minWidth: 85, fontSize: 11 }}>{formatDate(p.project_date)}</div>
                                                            <div className="notion-cell" style={{ minWidth: 85, fontSize: 11 }}>{formatDate(p.due_date)}</div>
                                                            <div className="notion-cell" style={{ minWidth: 80 }}>
                                                                {(p as any).invoice_status ? <span className="notion-badge badge-gray">{(p as any).invoice_status}</span> : '\u2014'}
                                                            </div>
                                                            <div className="notion-cell" style={{ minWidth: 70 }}>{(p as any).drive || '\u2014'}</div>
                                                            <div className="notion-cell" style={{ minWidth: 80, fontSize: 11 }}>{(p as any).size_in_gbs || '\u2014'}</div>
                                                            <div className="notion-cell" style={{ minWidth: 80 }}>
                                                                {(p as any).am_review ? <span className={`notion-badge ${(p as any).am_review === 'No Issue' ? 'badge-green' : 'badge-yellow'}`}>{(p as any).am_review}</span> : '\u2014'}
                                                            </div>
                                                            <div className="notion-cell" style={{ minWidth: 60 }}>{(p as any).code || '\u2014'}</div>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                        ...
                                        <PaginationControls
                                            ...
                                            pageSize={100}
                                            onGoToPage={(page) => { setCurrentPage(page); loadData(); }}
                                        />
                                    </div>
                                ) : viewMode === 'grid' ? (
