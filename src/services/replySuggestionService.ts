...
export async function generateReplySuggestion(
    ...
    try {
        let out = await callClaude(SYSTEM_PROMPT, userPrompt, controller.signal);
        if (!out) {
            out = await callGroq([
        ...
        if (!out) return { suggestion: null, modeSource, error: 'Claude, Groq, and Gemini all failed to generate a draft. Check API keys and rate limits.' };
